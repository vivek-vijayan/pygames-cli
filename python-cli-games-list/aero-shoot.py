...
# %% Intro Screen
INTRO_SCREEN_LOAD =pg.image.load("aero-shoot-supports/introscreen.png")
INTRO_SCREEN = pg.transform.scale(INTRO_SCREEN_LOAD, (WIDTH, HEIGHT))
...
def draw_intro_window(show:bool,screenObject):
    if show:
        WIN.blit(screenObject['source'],(screenObject['x-param'],screenObject['y-param']))
        pg.display.update()

# %% Moving intro screen
def move_intro_screen(screenObject:dict) -> int:
    if screenObject['x-param'] < 600:
        draw_intro_window(True, screenObject)
        return screenObject['y-param'] + 10
    return 1000
...
def main():
    ...
    while gamerun:
        ...
        # end line refresher with updates
            ...
            intro_screen_rect['y-param'] = move_intro_screen(intro_screen_rect)
            draw_game_window(1, spaceship_parameter, bullets, aliens, score_gained, lives_pending)
            
            
        else:
            draw_intro_window(showIntro, intro_screen_rect)
